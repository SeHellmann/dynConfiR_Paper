...
for (cur_model in models) {
  setwd(paste0("saved_details/fit_results/gen_model_", cur_model))
  file_list <- list.files(recursive = FALSE, full.names = FALSE, pattern = "RData")
  for (filei in file_list) {
    load(filei)
    if (nrow(pars)>0 & length(setdiff(names(pars), names(out)))>0) {
      pars[,setdiff(names(out), names(pars))] <- NA
      out[,setdiff(names(pars), names(out))] <- NA
    }
    pars <- rbind(pars, out)
  }
  setwd("../../../")  
...
for (cur_model in unique(collected_par_recovery$model)) {
  for (cur_ntrials in unique(collected_par_recovery$ntrials)) {
    p_rec_plt <- ggplot(subset(collected_par_recovery, model==cur_model & ntrials==cur_ntrials), aes(x=true, y=est))+
      geom_smooth(data=subset(collected_par_recovery, model==cur_model&ntrials==cur_ntrials), method="lm", alpha=0.5, size=0.9,
