...
temp_fct <- function() {
  ...
  fits_Law <- subset(parfits, model %in% c("dynaViTE", "IRMt", "PCRMt"))
  ...
  fits_dynaViTE <- subset(fits, model=="dynaViTE")
  ...
  fits_dynavite_Law <- fits_Law %>% subset(model=="dynaViTE")  %>%
    select(where(~!all(is.na(.x)))) %>%
    mutate(participant = participant + 400)
  ...
  save(fits_IRMt, fits_PCRMt, fits_dynaViTE, file="helper_fcts/collected_fits_models.RData")
  return(list(dynaViTE = fits_dynaViTE, IRMt = fits_IRMt, PCRMt=fits_PCRMt))
}
fits <- temp_fct()
rm(temp_fct)
